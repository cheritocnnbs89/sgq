...
from .parameters_constants import (
    ...
    GRUPO_OBLIGACIONES_FRECUENCIAS,
...
def register_parametros_generales_routes(app):
    ...
    def parametro_items(group_id):
        ...
        arbol = service.obtener_arbol_valores(group_id)
    ...
    def nuevo_valor(group_id):
        ...
        return render_template(
            TEMPLATE_NUEVO_VALOR,
            grupo=grupo,
            padres=service.listar_padres_candidatos(group_id),
            parent_id_prefill=request.args.get('parent_id'),
            grupo_frecuencias=GRUPO_OBLIGACIONES_FRECUENCIAS,
            frecuencias_opciones=FRECUENCIAS_VALOR_OPCIONES,
            usuario=session.get('usuario'),
            rol=session.get('rol'),
            active_page=ACTIVE_PAGE_PARAMETROS_GENERALES
        )
    ...
    def editar_valor(group_id, item_id):
        ...
        padres = [p for p in service.listar_padres_candidatos(group_id) if p['id'] != item_id]
