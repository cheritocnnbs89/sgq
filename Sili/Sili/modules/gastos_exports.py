...
def _build_gastos_workbook():
    ...
    - Si llega ?ids=1,2,3 exporta SOLO esos IDs.
    - Valida que esos IDs estén dentro del scope del rol.
    - Si no llega ids, exporta por filtros normales.
    - Mantiene plantilla, cabecera, título dinámico, CCB, totales y estilos.
    """
    ...
    def _sql_motivo_centro_select():
        """
        Fragmento compatible con SQL Server.
        Reemplaza:
        - TRIM(...) por LTRIM(RTRIM(...))
        - LIMIT 1 por SELECT TOP 1
        """
        return """
            COALESCE(
                NULLIF(LTRIM(RTRIM(COALESCE(g.motivo,''))), ''),
                (
                    SELECT TOP 1 d.motivo
                    FROM gastos_tarjeta_detalle d
                    WHERE d.gasto_id = g.id
                      AND LTRIM(RTRIM(COALESCE(d.motivo,''))) <> ''
                    ORDER BY d.id
                )
            ) AS motivo_resuelto,

            COALESCE(
                NULLIF(LTRIM(RTRIM(COALESCE(g.centro_costo,''))), ''),
                (
                    SELECT TOP 1 d.centro_costo
                    FROM gastos_tarjeta_detalle d
                    WHERE d.gasto_id = g.id
                      AND LTRIM(RTRIM(COALESCE(d.centro_costo,''))) <> ''
                    ORDER BY d.id
                )
            ) AS centro_resuelto,
        """
    ...
    try:
        # ======================================================
        # 0) IDs visibles desde UI
        # ======================================================
        ids_req = _parse_ids_req()
        ...
        if ids_req:
            ...
            if not ids_ok:
                ...
                # Mantiene el orden visible en la tabla.
                order_case = " ".join(
                    [f"WHEN ? THEN {i}" for i, _id in enumerate(ids_ok)]
                )
                ...
                sql_args = list(ids_ok) + list(ids_ok)

                current_app.logger.warning("[EXPORT_IDS] SQL=%s", sql)
                current_app.logger.warning("[EXPORT_IDS] ARGS=%s", sql_args)

                cur.execute(sql, sql_args)
                rows = cur.fetchall()

        # ======================================================
        # 6) Query normal por filtros
        # ======================================================
        else:
            sql = f"""
                SELECT
            ...
            current_app.logger.warning("[EXPORT_FILTERS] SQL=%s", sql)
            current_app.logger.warning("[EXPORT_FILTERS] ARGS=%s", args)
        ...
        # No cerrar todavía: conn se usa para cabecera si hay plantilla.
        pass
        ...
        try:
            ...
            if aprob_dt:
                ...
                if gerente_id:
                    ...
                    cur2.execute(
                        """
                        SELECT TOP 1 COALESCE(nombre_completo, username, '') AS n
                        FROM usuarios
                        WHERE id = ?
                        """,
                        (int(gerente_id),)
            ...
            current_app.logger.exception("[EXPORT] no se pudo llenar cabecera")

        try:
            titulo = _build_report_title_from_rows(rows)
            _set_report_title(ws, titulo)
        except Exception:
            current_app.logger.exception("[EXPORT] no se pudo cambiar título")
