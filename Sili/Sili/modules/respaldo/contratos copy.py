...
def _ensure_contratos_columns() -> None:
    conn = get_db()
    _add_column_if_missing(conn, "contratos", "usuario_solicitante_id", "INTEGER")
    _add_column_if_missing(conn, "contratos", "usuario_compras_id", "INTEGER")
    _add_column_if_missing(conn, "contratos", "aprobado_jefe", "INTEGER DEFAULT 0")
    _add_column_if_missing(conn, "contratos", "aprobado_jefe_por", "INTEGER")
    _add_column_if_missing(conn, "contratos", "aprobado_jefe_en", "TEXT")
    _add_column_if_missing(conn, "contratos", "status_interno", "TEXT DEFAULT 'No aprobado'")
    _add_column_if_missing(conn, "contratos", "departamento_id", "INTEGER")
    _add_column_if_missing(conn, "contratos", "creado_por", "INTEGER")
    _add_column_if_missing(conn, "contratos", "actualizado_at", "TEXT")
    _add_column_if_missing(conn, "contratos", "usuario_compras_nombre", "TEXT")


def _ensure_garantias_columns() -> None:
    conn = get_db()
    _add_column_if_missing(conn, "garantias", "compania_emisora", "TEXT")
    _add_column_if_missing(conn, "garantias", "monto_poliza", "REAL")
    _add_column_if_missing(conn, "garantias", "fecha_suscripcion", "TEXT")
    _add_column_if_missing(conn, "garantias", "fecha_vencimiento", "TEXT")
    _add_column_if_missing(conn, "garantias", "fecha_vencimiento_actual", "TEXT")
    _add_column_if_missing(conn, "garantias", "vigencia_dias", "INTEGER")
    _add_column_if_missing(conn, "garantias", "estado", "TEXT")
    _add_column_if_missing(conn, "garantias", "fecha_renovacion", "TEXT")
    _add_column_if_missing(conn, "garantias", "requiere_renovacion", "INTEGER DEFAULT 0")
    _add_column_if_missing(conn, "garantias", "status_interno", "TEXT DEFAULT 'No aprobado'")
    _add_column_if_missing(conn, "garantias", "observaciones", "TEXT")
    # Aprobaciones (Gerencia ya usa 'aprobado', garantizamos su existencia)
    _add_column_if_missing(conn, "garantias", "aprobado", "INTEGER DEFAULT 0")
    _add_column_if_missing(conn, "garantias", "aprobado_por", "INTEGER")
    _add_column_if_missing(conn, "garantias", "aprobado_en", "TEXT")
    # Aprobación de Jefe exclusiva de Garantías
    _add_column_if_missing(conn, "garantias", "aprobado_jefe", "INTEGER DEFAULT 0")
    _add_column_if_missing(conn, "garantias", "aprobado_jefe_por", "INTEGER")
    _add_column_if_missing(conn, "garantias", "aprobado_jefe_en", "TEXT")
    _add_column_if_missing(conn, "garantias", "actualizado_at", "TEXT")


def _ensure_softdelete_columns():
    conn = get_db()
    _add_column_if_missing(conn, "contratos", "disabled", "INTEGER DEFAULT 0")
    _add_column_if_missing(conn, "garantias", "disabled", "INTEGER DEFAULT 0")
...
def _list_contratos_aprobados_jefe(conn, limit: int = 200, include_id: int | None = None):
    ...
    Si la columna aprobado_jefe no existe (instalaciones viejas), usa status_interno='Aprobado'.
    include_id: asegura incluir un contrato específico aunque no cumpla el filtro (útil en editar).
    """
    tiene_aprob_jefe = _has_column(conn, "contratos", "aprobado_jefe")
    top_n = max(1, int(limit or 200))
    sql = f"""
        SELECT TOP {top_n} id, pedido, proveedor
          FROM contratos
         WHERE COALESCE(disabled,0)=0
    """
    if tiene_aprob_jefe:
        sql += " AND COALESCE(aprobado_jefe,0)=1"
    else:
        sql += " AND status_interno='Aprobado'"
    ...
    if include_id:
        cur = conn.cursor()
        extra = cur.execute(
            "SELECT TOP 1 id, pedido, proveedor FROM contratos WHERE id=?",
            (include_id,)
        ).fetchone()
...
def _list_contratos_aprobados_sin_garantia(conn, limit: int = 200, include_id: int | None = None):
    ...
    sql = f"""
        ...
        WHERE COALESCE(c.disabled,0)=0
          {"AND COALESCE(c.aprobado_jefe,0)=1" if tiene_aprob_jefe else "AND c.status_interno='Aprobado'"}
    ...
    if include_id:
        extra = cur.execute(
            "SELECT TOP 1 id, pedido, proveedor FROM contratos WHERE id=? AND COALESCE(disabled,0)=0",
            (include_id,),
        ).fetchone()
        if extra and not any(r["id"] == extra["id"] for r in rows):
            rows = [extra] + list(rows)
...
def compras_lista():
    ...
    tiene_aprob_jefe = _has_column(conn, "contratos", "aprobado_jefe")
    col_aprob_jefe = "COALESCE(c.aprobado_jefe,0) AS aprobado_jefe" if tiene_aprob_jefe else "0 AS aprobado_jefe"
