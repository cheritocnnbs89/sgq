...
def _build_conn_str(database: Optional[str] = None) -> str:
    host = os.environ.get("SQLSERVER_HOST", "172.16.40.53").strip()
...
def set_config_values(data: Dict[str, str]):
    ...
    try:
        ...
        for clave, valor in data.items():
            ...
            row = cur.fetchone()
            if row:
                cur.execute("UPDATE configuracion SET valor = ? WHERE clave = ?", (valor, clave))
            else:
                cur.execute("INSERT INTO configuracion (clave, valor) VALUES (?, ?)", (clave, valor))
        conn.commit()
    except Exception:
        try:
            conn.rollback()
