...
@require_login
@require_permission("scheduler", "ver")
...
def scheduler_toggle(job_key):
    if not has_permission(session.get("rol"), "scheduler", "editar"):
...
def scheduler_run_now(job_key):
    if not has_permission(session.get("rol"), "scheduler", "aprobar"):
        return jsonify(ok=False, error="Sin permiso para ejecutar manualmente"), 403
...
def _run_job(job_key: str) -> str:
    ...
    runners = {
        ...
        "obligaciones_notificaciones": _run_obligaciones,
    }
