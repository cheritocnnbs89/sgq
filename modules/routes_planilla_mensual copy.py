...
def ensure_schema(conn):
    ...
    # Responsables
    cur.execute("""
        CREATE TABLE IF NOT EXISTS plan_responsables(
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            nombre TEXT NOT NULL UNIQUE
        )
    """)
    # Tareas (con departamento_id)
    cur.execute(f"""
        CREATE TABLE IF NOT EXISTS plan_tareas(
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            nombre TEXT NOT NULL,
            frecuencia TEXT NOT NULL CHECK(frecuencia IN {FRECUENCIAS}),
            responsable_id INTEGER NOT NULL REFERENCES plan_responsables(id) ON DELETE CASCADE,
            departamento_id INTEGER REFERENCES departamentos(id),
            activo INTEGER NOT NULL DEFAULT 1
        )
    """)
    # Si es tabla vieja sin columna departamento_id -> agregar
    try:
        cur.execute("SELECT departamento_id FROM plan_tareas LIMIT 1")
    except Exception:
        cur.execute("ALTER TABLE plan_tareas ADD COLUMN departamento_id INTEGER NULL")

    # Checks por día
    cur.execute("""
        CREATE TABLE IF NOT EXISTS plan_checks(
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            tarea_id INTEGER NOT NULL REFERENCES plan_tareas(id) ON DELETE CASCADE,
            fecha DATE NOT NULL,
    ...
    # Config notificación por departamento
    cur.execute("""
        CREATE TABLE IF NOT EXISTS plan_dept_config(
            departamento_id INTEGER PRIMARY KEY REFERENCES departamentos(id) ON DELETE CASCADE,
            notify_user_id INTEGER REFERENCES usuarios(id),
            notify_email TEXT
        )
    """)
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS plan_mail_templates(
    ...
    """)
 # 1) Crear tabla (sin expresiones en PRIMARY/UNIQUE)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS plan_feriados (
            id     INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    # 2) Migración de datos antiguos: normalizar pais a 'EC' si viene NULL o vacío
    cur.execute("UPDATE plan_feriados SET pais='EC' WHERE pais IS NULL OR TRIM(pais)='';")

    # 3) Índice único (fecha, pais) - válido en SQLite
    cur.execute("""
        CREATE UNIQUE INDEX IF NOT EXISTS ux_plan_feriados_fecha_pais
        ON plan_feriados (fecha, pais)
    """)

    # Plantilla por defecto para evidencia
    row = cur.execute("SELECT 1 FROM plan_mail_templates WHERE key='evidencia'").fetchone()
    if not row:
        default_subject = 'Evidencia registrada: {{ tarea }} ({{ fecha }})'
        default_html = (
            "<p>Se registró evidencia para la tarea "
            "<strong>{{ tarea }}</strong> el {{ fecha }}.</p>"
            "<p><strong>Departamento:</strong> {{ departamento }}<br>"
            "<strong>Responsable:</strong> {{ responsable }}</p>"
            "{% if observacion %}"
            "<p><strong>Observación:</strong><br>{{ observacion|e }}</p>"
            "{% endif %}"
            "<p>Generado por {{ app_url }}</p>"
        )
        default_text = (
            "Evidencia registrada para {{ tarea }} el {{ fecha }}. "
            "Departamento: {{ departamento }}. Responsable: {{ responsable }}. "
            "{% if observacion %}Obs: {{ observacion }}{% endif %}"
        )
        cur.execute(
            "INSERT INTO plan_mail_templates(key, subject, html, text) VALUES (?,?,?,?)",
            ("evidencia", default_subject, default_html, default_text)
        )

    # Semillas mínimas responsables
    row = cur.execute("SELECT COUNT(*) AS c FROM plan_responsables").fetchone()
    try:
        count = row["c"]
    except Exception:
        count = row[0]
    if count == 0:
        cur.executemany(
            "INSERT INTO plan_responsables(nombre) VALUES (?)",
            [("Pedro Palma",), ("Jorge Chávez",)]
        )

    # --- columnas nuevas: dia_semana (0=Lunes..6=Domingo), dia_mes (1..31)
    try:
        cur.execute("SELECT dia_semana, dia_mes FROM plan_tareas LIMIT 1")
    except Exception:
        try:
            cur.execute("ALTER TABLE plan_tareas ADD COLUMN dia_semana INTEGER NULL")
        except Exception:
            pass
        try:
            cur.execute("ALTER TABLE plan_tareas ADD COLUMN dia_mes INTEGER NULL")
        except Exception:
            pass
...
def planilla_dashboard():
    ...
    conn = get_db(); ensure_schema(conn); cur = conn.cursor()
    ...
    # NORMALIZAR CAMPOS
    for t in tareas:
        t["frecuencia"] = (t.get("frecuencia") or "").strip().lower()  # 'diario'|'semanal'|'mensual'
        # Asegurar enteros o None
    ...
    pais = 'EC'  # opcional: traer de sesión/config
    ...
    def _is_weekend(d: date) -> bool:
      return d.weekday() in (5, 6)  # 5=Sab, 6=Dom
    ...
    def _effective_cutoff(today_: date, workdays_list):
        """
        Devuelve el último día laborable <= hoy dentro del mes; si no hay, None.
        workdays_list: lista de días laborables del mes (ya sin sáb/dom/feriados)
        """
        cand = [d for d in workdays_list if d <= today_]
        return cand[-1] if cand else None

    # ===== Programación diaria (EXCLUYENDO sábados y domingos) =====
    ...
    def _occurs_this_day(t, d: date) -> bool:
        # No contar sábados, domingos ni feriados para el planeado
        if _is_non_working_day(d):
            return False

        f = (t.get("frecuencia") or "").lower()
        if f == "diario":
            return True  # sólo días laborables por regla superior

        if f == "semanal":
            ds = t.get("dia_semana")
            if ds is None:
                ds = 0  # por defecto Lunes
            # Normaliza 1..7 -> 0..6 si tus datos vienen en 1=lun,...7=dom
            if 1 <= ds <= 7:
                ds = (ds - 1)
            # Evita programar sábado/domingo por si llegan así
            if ds in (5, 6):
                return False
            return d.weekday() == ds
        ...
        if f == "mensual":
            ...
            if dm is None:
                return d.day == 1
            # Asegura que no pase del último día del mes
            dm = min(int(dm), _last_day_of_month(d.year, d.month))
            return d.day == dm

        # 'todos' u otros: no programado explícito
        return False

    def _planned_counts_by_day(tareas_list, days_list):
        out = defaultdict(int)
        for d in days_list:
            if _is_non_working_day(d):  # <--- usa feriados + fines de semana
                continue
    ...
    # Para gráficos (etiquetas y acumulados) y proyección:
    workdays_all = [d for d in dias if not _is_non_working_day(d)]
    cutoff_eff   = _effective_cutoff(today, workdays_all)  # <-- NUEVO

    # Si no hay laborables antes de hoy (p.ej. mes empezó en sábado/domingo),
    # usamos el primer laborable para evitar divisiones por cero en la proyección.
    ...
    # *** Planeadas POR TAREA (usa _occurs_this_day → respeta no laborables) ***
    ...
    # ===== Realizados del mes (por tarea) — filtrado =====
    # Trae todos los checks del mes con fecha
    rows_checks_mes = cur.execute(f"""
        SELECT tarea_id, COALESCE(DATE(fecha), substr(fecha,1,10)) AS f
        FROM plan_checks
        WHERE fecha BETWEEN ? AND ? AND COALESCE(checked,1)=1
        AND tarea_id IN ({ph})
    """, (ini, fin, *task_ids)).fetchall()

    # Indexa por tarea y filtra con _occurs_this_day (respeta no laborables y reglas de la tarea)
    from collections import defaultdict as _dd
    realizados_por_tarea = _dd(int)
    date_cache = {}  # evita recrear objetos date
    for r in rows_checks_mes:
        ...
        # cuenta solo si ese día era laborable y la tarea ocurría ese día
    ...
    if cutoff_eff:
        ...
        for r in rows_hoy:
            ...
            # solo cuenta si el día es laborable y la tarea ocurre ese día
            if (not _is_non_working_day(d)):
                t = next((tt for tt in tareas if tt["id"] == tid), None)
                if t and _occurs_this_day(t, d):
    ...
    cumplimiento = (100.0 * total_real / total_plan) if total_plan else 0.0
    ...
    # ===== Real por día (filtrado SQL, pero graficaremos solo laborables) =====
    day_rows_all = cur.execute(f"""
        SELECT COALESCE(DATE(fecha), substr(fecha,1,10)) AS f, COUNT(*) AS n
        FROM plan_checks
        WHERE fecha BETWEEN ? AND ? AND COALESCE(checked,1)=1
        AND tarea_id IN ({ph})
        GROUP BY f
    ...
    # ===== A la fecha (corte hoy) =====
    # Usa el último laborable (cutoff_eff); si None, queda todo en 0 “a hoy”
    if cutoff_eff:
        ce = cutoff_eff.isoformat()
    ...
    # === Totales a hoy usando la MISMA base que la tabla ===
    ...
    # Esperado a hoy: baseline acumulada / plan mensual total
    esperado_hoy_pct = (100.0 * planeados_hoy_total / (total_plan or 1)) if total_plan else 0.0

    # Real a hoy: sobre lo planeado a la fecha (mismo criterio que la tabla)
    ...
    # Pendientes HOY (si es mes actual; si cae fin de semana, será 0)
    pendientes_hoy = 0
    ...
    # ===== Agregados por depto / responsable =====
    by_depto = {}
    for t in tareas:
    ...
    # ===== Gráficos (solo laborables en trend y burnup) =====
    # Tendencia diaria (real) SOLO días laborables
    trend_labels = [d.isoformat() for d in workdays_all]
    trend_real   = [real_per_day.get(x, 0) for x in trend_labels]

    # Burn-up SOLO días laborables
    plan_acum, real_acum, burn_labels = [], [], []
    acc_p = acc_r = 0

    for d in workdays_all:  # solo laborales (ya excluyen sáb/dom/feriados)
        k = d.isoformat()
        # Planeado acumulado (baseline)
        acc_p += planned_per_day.get(k, 0)
        plan_acum.append(acc_p)

        # Real acumulado: solo suma si d <= corte efectivo
        if cutoff_eff and d <= cutoff_eff:
            acc_r += real_per_day.get(k, 0)
        # Si no hay corte o es futuro, se mantiene plano
    ...
    # ===== Proyección usando días laborables (global) =====
    wd_total = len(workdays_all)
    wd_elapsed = len(workdays_elapsed)
    #avg_real_per_workday = (realizados_hoy / wd_elapsed) if wd_elapsed else 0.0
    #proy_mes_real = avg_real_per_workday * wd_total
    #proyeccion_pct = (100.0 * proy_mes_real / (total_plan or 1)) if total_plan else 0.0
    # Proyección fin de mes consistente con lo anterior
    avg_real_per_workday = (realizados_hoy_total / (wd_elapsed or 1))
    proy_mes_real        = avg_real_per_workday * wd_total
    proyeccion_pct       = (100.0 * proy_mes_real / (total_plan or 1)) if total_plan else 0.0
    # ===== Resumen por (Departamento, Responsable) =====
    # ===== Resumen por (Departamento, Responsable) =====
    # ===== Resumen por (Departamento, Responsable) =====
    from collections import defaultdict as _dd
    grp = _dd(lambda: {
    ...
    for (dep, resp), g in grp.items():
        ...
        # % ESPERADO A HOY (baseline acumulada vs plan mensual)
        esperado_hoy_pct = 100.0 * plan_hoy / plan_mes if plan_mes else 0.0

        # % REAL A HOY (sobre lo planeado a la fecha)
        cumpl_a_hoy_pct = 100.0 * real_hoy / plan_hoy if plan_hoy else (100.0 if real_hoy > 0 else 0.0)

        # GAP = real a hoy %  –  esperado a hoy %
        #gap_pct = cumpl_a_hoy_pct - esperado_hoy_pct
        #gap_pct = esperado_hoy_pct - cumpl_mes_pct
        gap_pct = cumpl_mes_pct - esperado_hoy_pct



        # Proyección fin de mes (misma lógica que la tarjeta)
        ...
        summary_rows.append({
            ...
            "cumpl_mes_pct": cumpl_mes_pct,          # ← nuevo nombre claro
            "plan_hoy": plan_hoy, "real_hoy": real_hoy,
            "esperado_hoy_pct": esperado_hoy_pct,    # ← EXPUESTO PARA LA TABLA
            "cumpl_a_hoy_pct": cumpl_a_hoy_pct,      # ← EXPUESTO PARA LA TABLA
            "gap_pct": gap_pct,
            "proy_pct": proy_pct
        })
    ...
    summary_rows.sort(key=lambda r: ((r["depto"] or "").lower(), (r["resp"] or "").lower()))
        ...
        # ===== KPIs =====
    # --- NUEVO: define ambas métricas con nombre claro ---
    # Avance del mes completo (lo que hoy aparece como “Cumplimiento”)
    cumplimiento_mes = (100.0 * total_real / (total_plan or 1)) if total_plan else 0.0

    # Cumplimiento a la fecha (real acumulado a hoy / planeado a hoy)
    if planeados_hoy_total:
    ...
    # --- CAMBIA el dict kpis para “cruzar” los valores ---
    kpis = dict(
        # “Cumplimiento” ahora muestra el real/plan A LA FECHA
        cumplimiento=cumplimiento_a_hoy,

        # “Cumplimiento real a hoy” (la tarjeta de la fila 2) mostrará el AVANCE DEL MES
        ...
        esperado_hoy_pct=esperado_hoy_pct,  # baseline a la fecha (debe ser mayor en tu caso)
        backlog=backlog,
    ...
    def months_to_year_end(y0, m0):
        """Devuelve [(y,m), ...] desde (y0,m0) hasta diciembre de y0."""
    ...
    # -------- Proyección "Resto del año" para la tabla nueva ----------
    proj_rows = []
    proj_cols = []  # columnas dinámicas: [{'key':'m_10','label':'Oct %'}, ...]
    months_left = months_to_year_end(y, m)

    # ritmo del grupo se toma del mes visible (real_hoy / wd_elapsed), igual que arriba
    for (dep, resp), g in grp.items():
        ...
        ATTN_THRESHOLD = 60.0   # ← nuevo: mínimo para "Requiere atención"
        WARN_HIGH      = 85.0   # rango de “observación” (puedes ajustarlo)
        GAP_RED        = -10.0  # atraso fuerte

        first_key = f"m_{months_left[0][0]}_{months_left[0][1]}"
        first_month_pct = row.get(first_key, 100)

        reds = 0
        # 1) Atraso fuerte por GAP
        reds += 1 if gap_pct_group <= GAP_RED else 0
        # 2) Proyección del mes visible por debajo del mínimo aceptable
        reds += 1 if (first_month_pct < ATTN_THRESHOLD) else 0
        # 3) PROMEDIO RESTO-AÑO por debajo del mínimo aceptable (AQUÍ SE USA EL 60%)
        reds += 1 if (q_total_pct < ATTN_THRESHOLD) else 0

        warns = 0
        # Observación si hay atraso leve o proyecciones en “zona amarilla”
    ...
    if checked:
        if evidence_mode:
            ...
            cur.execute("""
                ...
                VALUES (?,?,1)
                ON CONFLICT(tarea_id, fecha) DO UPDATE SET checked=1
            """, (tarea_id, fecha))
            conn.commit()
...
def evidence_post():
    ...
    cur.execute("""
        ...
        ON CONFLICT(tarea_id, fecha) DO UPDATE SET
            obs=excluded.obs,
            file_path=COALESCE(excluded.file_path, plan_evidencias.file_path),
            file_name=COALESCE(excluded.file_name, plan_evidencias.file_name),
            mime=COALESCE(excluded.mime, plan_evidencias.mime)
...
def tareas_new():
    ...
    if responsable_user_id:
        ...
        cur.execute("INSERT OR IGNORE INTO plan_responsables(nombre) VALUES (?)", (uname,))
        row_r = cur.execute("SELECT id FROM plan_responsables WHERE nombre=?", (uname,)).fetchone()
        responsable_id = row_r["id"] if hasattr(row_r, "keys") else row_r[0]
    elif nuevo_resp:
        cur.execute("INSERT OR IGNORE INTO plan_responsables(nombre) VALUES (?)", (nuevo_resp,))
        row_r = cur.execute("SELECT id FROM plan_responsables WHERE LOWER(nombre)=LOWER(?)", (nuevo_resp,)).fetchone()
        responsable_id = row_r["id"] if hasattr(row_r, "keys") else row_r[0]
    else:
...
def mail_template():
    ...
    conn = get_db(); ensure_schema(conn)
    if request.method == "GET":
        row = conn.execute("SELECT subject, html, text FROM plan_mail_templates WHERE key='evidencia'").fetchone()
...
def api_add_feriado():
    ...
    try:
        cur.execute("INSERT OR IGNORE INTO plan_feriados (fecha, nombre, pais) VALUES (?,?,?)",
                    (fecha, nombre, pais))
        conn.commit()
    except Exception as e:
