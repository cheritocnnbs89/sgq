...
def fetch_parent_candidates_by_group_id(group_id: int):
    """Valores 'padre' del grupo (parent_id NULL, activos) -- para el desplegable de Parent ID."""
    conn = get_db()
    cur = conn.cursor()
    cur.execute(q.SQL_SELECT_PARENT_CANDIDATES_BY_GROUP_ID, (group_id,))
    return cur.fetchall()
