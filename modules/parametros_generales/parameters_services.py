...
def _agrupar_arbol(rows):
    """Organiza filas planas (dict-like, con 'id'/'parent_id') en padres con lista 'hijos'."""
    padres = []
    hijos_por_padre = {}
    ...
    for r in rows:
        if r["parent_id"] is None:
            ...
            hijos_por_padre.setdefault(r["parent_id"], []).append(r)

    for p in padres:
        p["hijos"] = hijos_por_padre.get(p["id"], [])

    return padres


def obtener_arbol_valores(group_id: int):
    """Valores del grupo organizados padre -> hijos, para la vista maestro-detalle."""
    repo.ensure_param_tables()
    rows = repo.fetch_values_by_group_id(group_id)
    return _agrupar_arbol(rows)
...
def listar_padres_candidatos(group_id: int):
    """Valores 'padre' del grupo (sin parent_id propio) para el desplegable de Parent ID."""
    repo.ensure_param_tables()
    return repo.fetch_parent_candidates_by_group_id(group_id)
...
if __name__ == "__main__":
    # ponytail: self-check de _agrupar_arbol (no toca BD)
    _demo = [
        {"id": 1, "parent_id": None, "nombre": "Mensual"},
        {"id": 2, "parent_id": 1, "nombre": "30 dias antes"},
        {"id": 3, "parent_id": None, "nombre": "Anual"},
    ]
    _arbol = _agrupar_arbol(_demo)
    assert len(_arbol) == 2, "deben quedar 2 padres"
    assert _arbol[0]["hijos"][0]["nombre"] == "30 dias antes"
    assert _arbol[1]["hijos"] == []
    print("OK: _agrupar_arbol")